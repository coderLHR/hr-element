...
import { useId, useFocusController, useClickOutside } from '@hr-element/hooks'
...
import HrOption from './Option.vue'
import HrTooltip from '../Tooltip/Tooltip.vue'
import HrInput from '../Input/Input.vue'
import HrIcon from '../Icon/Icon.vue'
import { debugWarn, RenderVnode } from '@hr-element/utils'

const COMPONENT_NAME = 'HrSelect'
...
const children = computed(() =>
  filter(slots?.default?.(), (child) => eq(child.type, HrOption))
)
...
<template>
  <div ref="selectRef"
    ...
    <hr-tooltip ref="tooltipRef"
      ...
      <template #content>
        <div class="hr-select__loading"
             v-if="selectStates.loading">
          <hr-icon icon="spinner"
                   spin />
        </div>
        <div class="hr-select__nodata"
             v-else-if="filterable && isNoData">
          No data
        </div>
        <ul class="hr-select__menu">
          <template v-if="!hasChildren">
            <hr-option v-for="item in filteredOptions"
                       :key="item.value"
                       v-bind="item" />
          </template>
          <template v-else>
            <template v-for="[vNode, _props] in filteredChilds"
                      :key="_props.value">
