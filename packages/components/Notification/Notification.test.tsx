...
describe("Notification", () => {
  test("notification() function", async () => {
    ...
    await rAF();
    expect(document.querySelector(".hr-notification")).toBeTruthy();
    handler.close();
    await rAF();
    expect(document.querySelector(".hr-notification")).toBeFalsy();
  });
  ...
  test("call notification() function more than once", async () => {
    ...
    notification({ message: "hello notify", duration: 0 });
    await rAF();
    expect(document.querySelectorAll(".hr-notification").length).toBe(2);
    notification.closeAll();
    await rAF();
    expect(document.querySelectorAll(".hr-notification").length).toBe(0);
  });
  ...
  test("notification offset", async () => {
    ...
    await rAF();
    const elements = document.querySelectorAll(".hr-notification");
